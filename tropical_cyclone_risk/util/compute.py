...
def run_tracks(year, n_tracks, b):
    ...
    # Lista para almacenar las fechas dt_month que cumplan seed_passed
    passed_dates = []
    ...
    while nt < n_tracks:
        ...
        while not seed_passed:
            # Random genesis location for the seed.
            ...
            # Generar una fecha aleatoria dentro del mes seleccionado
            _, num_days_month = calendar.monthrange(year, month_seed)
            random_day = random.randint(1, num_days_month)
            dt_month = datetime.datetime(year, month_seed, random_day)

            fast = cpl_fast[month_seed - 1]
            if fast == 0:
                ds_dt_month = input.convert_from_datetime(ds_wnd, [dt_month])[0]

                # Interpolar los datos climáticos para el mes.
                ...
                # Interpolar los campos de mezcla.
                mld_month = mat.interp_2d_grid(mld['lon'], mld['lat'], np.nan_to_num(mld[:, :, month_seed - 1]), lon, lat)
                strat_month = mat.interp_2d_grid(strat['lon'], strat['lat'], np.nan_to_num(strat[:, :, month_seed - 1]), lon, lat)

                # Inicializar cpl_fast.
                cpl_fast[month_seed - 1] = coupled_fast.Coupled_FAST(fn_wnd_stat, b, ds_dt_month, namelist.output_interval_s, T_s)
                cpl_fast[month_seed - 1].init_fields(lon, lat, chi_month, vpot_month, mld_month, strat_month)
        ...
        if res != None:
            ...
            # The TC must also reach the genesis threshold during its entire lifetime.
        ...
        if is_tc:
            ...
            if np.nanmax(vmax) >= namelist.seed_vmax_threshold_ms:
                ...
                passed_dates.append(dt_month)  # Guardar la fecha del mes generada para este ciclón
                nt += 1

    # Convertimos la lista de fechas a un numpy array para poder guardarlo
    passed_dates_np = np.array(passed_dates)

    # Creamos un Dataset de xarray para las fechas
    ds_dates = xr.Dataset({
        'passed_dates': (['time'], passed_dates_np)})

    # Guardar el dataset en un archivo NetCDF
    output_filename = f'passed_dates_{year}.nc'
    ds_dates.to_netcdf(output_filename)

    return (tc_lon, tc_lat, tc_v, tc_m, tc_vmax, tc_env_wnds, tc_month, tc_basin, n_seeds, passed_dates)
...
def run_downscaling(basin_id):
    ...
    ds = xr.Dataset(data_vars = dict(lon_trks = total_time_s,
                                     ...
                                     #total_time_s = (["time"], total_time_s),
